...
class ClientForm(CenteredWindow):
    def __init__(self, title):
        ...
        self.__entry_rsocial = Entry(self, textvariable=self.__str_rsocial)
        self.__entry_nfantasia = Entry(self, textvariable=self.__str_nfantasia)
        self.__entry_cnpj = Entry(self, textvariable=self.__str_cnpj)
        self.__entry_iestadual = Entry(self, textvariable=self.__str_iestadual)
        self.__entry_imunicipal = Entry(self, textvariable=self.__str_imunicipal)
        self.__entry_logradouro = Entry(self, textvariable=self.__str_logradouro)
        self.__entry_complemento = Entry(self, textvariable=self.__str_complemento)
        self.__entry_bairro = Entry(self, textvariable=self.__str_bairro)
        self.__entry_municipio = Entry(self, textvariable=self.__str_municipio)
        self.__entry_uf = Entry(self, textvariable=self.__str_uf)
        self.__entry_cep = Entry(self, textvariable=self.__str_cep)
        self.__entry_telefone = Entry(self, textvariable=self.__str_telefone)
        self.__entry_ncel = Entry(self, textvariable=self.__str_ncel)
        
        self.__check_whatsapp = Checkbutton(self, text='WhatsApp?', 
                variable=self.__int_whatsapp, width=9)

        self.__entry_email = Entry(self, textvariable=self.__str_email)
        self.__entry_url = Entry(self, textvariable=self.__str_url)
        
        self.__button_salvar = Button(
                self, text="Salvar", command=self.destroy)
        self.__button_cancelar = Button(
                self, text="Cancelar", command=self.destroy)

        self.__rule_rsocial = [ ALPHANUMERIC + SPACE, 255 ]
        self.__rule_nfantasia = [ ALPHANUMERIC + SPACE, 255 ]
        self.__rule_cnpj = [ NUMERIC, 14, [2, '.'], [5, '.'], [8, '/'], [12, '-'] ]
        self.__rule_iestadual = [ NUMERIC, 12, [3, '.'], [6, '.'], [9, '.'] ]
        self.__rule_imunicipal = [ NUMERIC, 8, [1, '.'], [4, '.'], [7, '-'] ]
        self.__rule_logradouro = [ ALPHANUMERIC + SPACE, 255 ]
        self.__rule_complemento = [ ALPHANUMERIC + SPACE, 255 ]
        self.__rule_bairro = [ ALPHABET + SPACE, 255 ]
        self.__rule_municipio = [ ALPHABET + SPACE, 255 ]
        self.__rule_uf = [ ALPHABET + SPACE, 255 ]
        self.__rule_cep = [ NUMERIC, 8, [5, '-'] ]
        self.__rule_telefone = [ NUMERIC, 10, [0, '('], [2, ') '], [7, '-'] ]
        self.__rule_ncel = [ NUMERIC, 11, [0, '('], [2, ') '], [4, ' '], [8, '-'] ]
        self.__rule_email = [ ALPHANUMERIC + "_@-.", 255 ]
        self.__rule_url = [ ALPHANUMERIC + "_-./", 255 ]

        self.__tracer_rsocial = Tracer(self.__entry_rsocial, self.__rule_rsocial)
        self.__tracer_nfantasia = Tracer(self.__entry_nfantasia, self.__rule_nfantasia)
        self.__tracer_cnpj = Tracer(self.__entry_cnpj, self.__rule_cnpj)
        self.__tracer_iestadual = Tracer(self.__entry_iestadual, self.__rule_iestadual)
        self.__tracer_imunicipal = Tracer(self.__entry_imunicipal, self.__rule_imunicipal)
        self.__tracer_logradouro = Tracer(self.__entry_logradouro, self.__rule_logradouro)
        self.__tracer_complemento = Tracer(self.__entry_complemento, self.__rule_complemento)
        self.__tracer_bairro = Tracer(self.__entry_bairro, self.__rule_bairro)
        self.__tracer_municipio = Tracer(self.__entry_municipio, self.__rule_municipio)
        self.__tracer_uf = Tracer(self.__entry_uf, self.__rule_uf)
        self.__tracer_cep = Tracer(self.__entry_cep, self.__rule_cep)
        self.__tracer_telefone = Tracer(self.__entry_telefone, self.__rule_telefone)
        self.__tracer_ncel = Tracer(self.__entry_ncel, self.__rule_ncel)
        self.__tracer_email = Tracer(self.__entry_email, self.__rule_email)
        self.__tracer_url = Tracer(self.__entry_url, self.__rule_url)
